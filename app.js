...
var url = "https://en.wikipedia.org/w/api.php?action=query&list=search&srlimit=20&format=json&origin=*&srsearch=", wikiform = document.querySelector("form"), results = document.querySelector(".results"), error = document.querySelector(".error"), keyword = document.getElementById("search_value");
wikiform.addEventListener("submit", function(a) {
  a.preventDefault();
  (a = keyword.value) ? fetchWikis(a) : ($(".error").html("<h2 class='error'>enter proper keyword</h2>"), $(".error").fadeIn(1000), $(".error").fadeOut(4000));
});
var fetchWikis = function(a) {
  var b, c, d;
  return $jscomp.asyncExecutePromiseGeneratorProgram(function(e) {
    switch(e.nextAddress) {
      case 1:
        return e.setCatchFinallyBlocks(2), e.yield(fetch(url + "," + a), 4);
      case 4:
      ...
      case 5:
        ...
        console.log(d);
        if (1 > d.length) {
          return error.innerHTML = "<h2 class='error'>no results found</h2>", $(".error").fadeIn(1000), $(".error").fadeOut(4000), e["return"]();
        }
        renderResults(d);
        e.leaveTryBlock(0);
        break;
      case 2:
        return e.enterCatchBlock(), $(".error").html("<h2 class='error'>there was an error :( </h2>"), $(".error").fadeIn(1000), e["return"]();
    }
  });
  ...
  return $jscomp.asyncExecutePromiseGeneratorProgram(function(c) {
    b = a.map(function(d) {
      return "<a href=http://en.wikipedia.org/?curid=" + d.pageid + ' target="_blank">\n              <h4>' + d.title + "</h4>\n              <p>\n                " + d.snippet + "\n              </p>\n            </a><hr>";
    }).join("");
